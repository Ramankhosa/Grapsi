...
import { buildFallbackContextSummary, hasMeaningfulSectionContent, sectionContentHash } from '@/lib/reviewer/content';
import {
...
export default async function handler(
  ...
  try {
    ...
    // to re-review (the "review all again" run). The re-run wrapper used to
    // flip the row back to draft *before* calling here, so an interrupted run
    // left reviewed sections stranded as drafts with their old review
    // invisible to the report. Now status only changes when a review lands.
    const forceReview = req.body?.force === true;
    ...
    // "Review all again" passes skipIfUnchanged so a force re-run does not pay
    // for sections whose text is identical to what was already reviewed. The
    // hash is stamped into ai_review_json at review time; reviews that predate
    // it simply miss and re-review. An explicit single-section re-review does
    // not send the flag, so it always executes.
    if (req.body?.skipIfUnchanged === true && section.status === 'reviewed') {
      const storedHash = section.ai_review_json && typeof section.ai_review_json === 'object'
      ...
      if (storedHash && storedHash === sectionContentHash(section.user_input)) {
        ...
        return res.status(200).json({
          message: 'Section unchanged since its last review',
    ...
    // Fetch available context summaries — one row per title (the newest
    // reviewed version) and never this section's own title. Superseded v1
    // rows keep status 'reviewed', so without DISTINCT ON the v1 and v2
    // summaries of the same section both entered the prompt and contradicted
    // each other.
    ...
    try {
      ...
      // Records what text this review scored, so a later force re-run can
      // prove the content unchanged and keep this review instead of paying
      ...
      // Progress is recorded on success, not on attempt: written before the
      // model call it claimed a section was reviewed when the review failed.
      const progressState = {
      ...
      // + searches), so a five-section revision loop paid for five complete
      // reports. The stored report now simply goes stale: the final-review
      // page shows its stale banner with a Regenerate button, and the ATR
      // export regenerates before serving — a deliverable is never stale.
      // The response keeps `report_refreshed` for API compatibility.
      return res.status(200).json({
