...
- **504 Gateway Time-out on handoff** — the inline `generateGrantBlueprintWithLlm` call took 60–71s in production (GRANT_BLUEPRINT_GENERATE on gpt-5.2-thinking), blowing the ~60s reverse-proxy timeout: the browser saw a 504 while the server kept working (and billing), and user retries doubled the cost. Fixed by **removing the inline blueprint LLM from launch entirely** — the launch now persists the deterministic plan (`state.sectionPlan`, byte-identical to the LLM path's own fallback), finishing in seconds. The LLM only added literature dimensions / citation hints, which the fast path doesn't consume (lit search is locked pre-freeze) and which remain available via the blueprint's explicit `generate_dimensions` action. Do NOT re-add background enrichment via `generateBlueprintLiteratureDimensions` after launch: its `updateBlueprintPlan` resets `GrantSession.status` to `'BLUEPRINT'` and resets shadow-workspace section statuses — it would undo the DRAFTING bypass and race in-progress drafting.
- **Client launch recovery** (draft-zero `page.tsx`) — if the handoff response is lost (proxy timeout / network blip), the page re-checks `sessionStatus`; if the launch actually completed it replays the handoff (idempotent short-circuit returns the stored launch info instantly) and navigates instead of stranding the user. The read-only "already launched" banner now carries a **Continue drafting** button (→ `/draft-one`, or `workspace?stage=SECTION_DRAFTING` without the flag).

### 3.4c Compliance sanity + Draft One paper redesign (July 2026)

Production showed **0/24 sections passing agency rules** with unreadable findings. Three code-level causes, all fixed:

- **URLs inside rule text** — the guideline/template extractors (grounded LLMs) sometimes glue scroll-to-text source citations straight onto the prose (`...and referenceswww.icmr.gov.in/...pdf#:~:text=b,as%20therapeutics...`). New module `src/lib/grants/ruleText.ts` (`sanitizeGrantRuleText` / `splitGrantRuleTextIntoPoints` / `summarizeGrantRuleText`) strips scheme URLs, bare domain paths, text fragments, and percent-encoded runs. Applied at: guideline normalize (`fundingGuidelines/utils.ts normalizeRule` — runs on every read, so already-persisted packs are cleaned too; URL-only rules are dropped), template compile (`workspace.ts` pushItem), enrichment (`pickTopRuleTexts`), and read-time in Draft One (`normalizeDraftOneSection` — protects reports persisted before the fix).
- **Paragraph-sized required points** — a template item without discrete `requiredFacts` dumped its whole guidance paragraph as ONE `mustCover` entry, and guideline enumerations arrived as one enormous sentence; keyword matching could never cover them, so every draft failed. `splitGrantRuleTextIntoPoints` splits on sentences then top-level commas (parentheticals preserved) into ≤6 atomic checkable points — applied in the compliance contract's `requiredPoints` and the template `mustCover` fallback. `isPhraseCovered` also grades the coverage bar by phrase length (>12 tokens → 0.5, >24 → 0.45) since long rules are topic requirements, not verbatim phrases.
- **Placeholder failure messages** — hard checks were labeled `Guideline hard rule N` and `buildCheckFailure` printed the LABEL, telling the user nothing. Labels now carry a 110-char snippet of the actual rule, and failure messages interpolate the rule text.
- **Draft One redesign** — warm paper backdrop, numbered white sheets in strict `sectionOrder` (agency template order), content rendered through `MarkdownRenderer` (Palatino serif; no more raw `###`/`-` markdown; `[CITE:key]` anchors render as citation markers), compact expandable "Agency check" panel with sanitized truncated findings, margin-notes agency-rules rail, run progress bar, stone/ink palette.
- **Draft One state fixes** — (1) `handleRun` looped over a `sections` closure frozen at run start, so mid-run refreshes never reached later queue items; it now reads through a `sectionsRef` mirror. (2) A missing `draftingSessionId` no longer dead-ends into a "go open the blueprint" banner: the page self-heals by POSTing the idempotent, LLM-free `{action:'launch'}` once, then loads sections.
