...
function isGrantAiGeneratedSection(section: SectionPlanItem): boolean {
  return isGrantAppDraftSection(section)
    || (section.workflowMode === 'app_draft' && section.sectionType === 'budget_rows');
}

function isGrantManualSection(section: SectionPlanItem): boolean {
  return !isGrantAiGeneratedSection(section);
}
...
function getGrantOwnershipLabel(section: SectionPlanItem): string {
  return isGrantAiGeneratedSection(section) ? 'AI will draft' : 'Project team will manually fill';
}
...
function GrantBlueprintApplicationView({
  ...
  const aiSections = orderedSections.filter(isGrantAiGeneratedSection);
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div className="flex min-w-0 gap-3">
            ...
            <div className="mb-2 flex flex-wrap items-center gap-2">
              ...
              <Badge className={isGrantAiGeneratedSection(section) ? 'bg-emerald-100 text-emerald-800 hover:bg-emerald-100' : 'bg-amber-100 text-amber-900 hover:bg-amber-100'}>
          ...
          <div className="rounded-xl border border-indigo-100 bg-indigo-50/70 p-4 dark:border-indigo-950 dark:bg-indigo-950/20">
            ...
            {evidenceMapped ? (
              ...
              <p className="text-sm leading-relaxed text-slate-600 dark:text-slate-300">
                {isGrantAiGeneratedSection(section)
                  ? section.sectionType === 'budget_rows'
                    ? 'This structured budget is generated by the app from the extracted budget template without literature evidence mapping.'
