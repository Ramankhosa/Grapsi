...
import { archiveFundingCall, FundingImportError } from '@/lib/funding/ingestion-service'
...
export async function POST(request: NextRequest, { params }: { params: { callId: string } }) {
  ...
  try {
    const call = await archiveFundingCall(params.callId, auth.actor)
    ...
    console.error('[SuperAdmin/Funding/Calls/:callId/archive] POST error:', error)
    return NextResponse.json({ error: 'Failed to archive funding call' }, { status: 500 })
  }
}
