...
export async function PUT(
  ...
  try {
    ...
    const paperTypeCode = session.paperBlueprint?.paperTypeCode || session.paperType?.code || null;
    const sectionPlan = Array.isArray(session.paperBlueprint?.sectionPlan)
      ? session.paperBlueprint.sectionPlan as Array<Record<string, any>>
      : [];
    const grantSectionPlan = sectionPlan.find((section) => section.sectionKey === data.sectionKey);
    if (
      ...
      return NextResponse.json(
        { error: 'App draft grant sections must be generated by the grant drafting workflow.' },
        { status: 400 }
      );
    }
