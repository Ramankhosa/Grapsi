...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    const bundle = await fundingTemplateService.revertTemplate(params.id, body.revisionNo, auth.operator, body.changeNotes)
    return NextResponse.json(bundle)
  } catch (error) {
    if (error instanceof z.ZodError) {
      return NextResponse.json({ message: 'Invalid revert request', issues: error.flatten() }, { status: 400 })
    }
    return NextResponse.json({ message: error instanceof Error ? error.message : 'Failed to revert template' }, { status: 500 })
  }
}
