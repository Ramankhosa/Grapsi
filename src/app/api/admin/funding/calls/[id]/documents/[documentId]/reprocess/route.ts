...
export async function POST(
  ...
  try {
    const detail = await fundingDocumentService.reprocessDocument(params.id, params.documentId, auth.operator);
    return NextResponse.json(detail, { status: 202 });
  } catch (error) {
    return NextResponse.json(
      { message: error instanceof Error ? error.message : 'Failed to reprocess funding document' },
      { status: 500 }
    );
  }
}
