  ...
  try {
    ...
    if (updates.assigned_team_id) {
      const team = await prisma.team.findFirst({
        where: { id: updates.assigned_team_id, tenantId: user.tenant_id, isActive: true },
        select: { id: true }
      })
      if (!team) {
        return NextResponse.json(
          { code: 'INVALID_TEAM', message: 'Assigned team must be active and belong to your tenant' },
          { status: 400 }
        )
      }
    }
