...
export async function GET(request:NextRequest,{params}:{params:{report:string}}){
  if(!['pending','deadline-risk','school-coverage','opportunity-gaps','performance','weekly-review','coverage','outcomes'].includes(params.report))return NextResponse.json({error:'Report not found.'},{status:404})
