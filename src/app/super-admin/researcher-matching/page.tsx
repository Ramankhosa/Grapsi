...
export default function ResearcherMatchingPage() {
  ...
  const handleBackfill = useCallback(async () => {
    setBackfillStatus('Running embedding backfill...')
    try {
      ...
      if (res.ok) {
        ...
        for (const [key, val] of Object.entries(data.results || {})) {
          const r = val as any
          if (r.succeeded > 0) parts.push(`${key}: ${r.succeeded} embedded`)
        }
        setBackfillStatus(parts.length > 0 ? `Done: ${parts.join(', ')}` : 'Done (nothing to backfill)')
        fetchStats()
      } else {
    ...
    <div style={{ maxWidth: 1200, margin: '0 auto', padding: '24px 16px' }}>
      ...
      <div style={{ marginBottom: 24, display: 'flex', alignItems: 'center', gap: 12 }}>
        <button
          ...
          disabled={!!backfillStatus && backfillStatus.startsWith('Running')}
          style={{
