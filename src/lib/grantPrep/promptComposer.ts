...
import { stageHasCapturedContent } from './sessionState';
import type {
...
function buildCrossStageContext(
  ...
  const completedStages = GRANT_PREP_STAGE_LIBRARY.filter(
    (stage) =>
      stage.pickable &&
      stage.key !== currentStageKey &&
      stageStates[stage.key]?.enabled &&
      stageHasCapturedContent(stageStates[stage.key])
  );
  ...
  return completedStages
    .map((stage) => {
      ...
      const coveredPoints = state.points
        .map((point) => ({
          label: point.label,
...
function getResponseRules(engagementMode: GrantPrepEngagementMode, stageKey: GrantPrepStageKey) {
  ...
  if (stageKey === 'risk_and_ethics' || stageKey === 'budget_strategy') {
    return [
      'PROSE: Write 35-80 words across at most 2 short paragraphs.',
      'If there is a blocker, cite one rule and propose one next move.',
  ...
  // Guided and Hybrid modes — the main coaching turns
  const modeSpecific = engagementMode === 'hybrid'
    ? 'No bullets unless explicitly needed for clarity.'
    : 'Do not recap earlier stages unless the stage has changed.';

  return [
    'PROSE: Write 80-160 words across at most 2 short paragraphs.',
    'Cover VALIDATE, CHALLENGE, and CONNECT in the prose. Be substantive — help the user brainstorm, not just interrogate.',
    'If the user\'s last answer was weak, explain specifically what a reviewer would flag and why.',
    '',
    ...
    '  - Specific and grounded in the funding call, prior answers, or domain norms',
    '  - Meaningfully different from each other (different angles, methods, scopes)',
    'If one option is stronger than others from a reviewer perspective, briefly note why.',
    '',
    'WORD BUDGET: The prose portion is 80-160 words. The options are ADDITIONAL — do not count them toward the prose limit.',
    'Total response (prose + question + options) will typically be 200-350 words. That is fine.',
    modeSpecific,
  ].join('\n');
}
...
export function buildGrantPrepPrompt(input: {
  ...
  return [
    ...
    buildCrossStageContext(input.session.stageStates, input.stageKey),
