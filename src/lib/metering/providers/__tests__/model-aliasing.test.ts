...
describe('Provider model aliasing', () => {
  ...
  it('routes new model codes to the expected providers', () => {
    ...
    expect(getProviderFromModelCode('glm-5.2')).toBe('zhipu')
    ...
    expect(getProviderFromModelCode('glm-4.6v')).toBe('zhipu')
    expect(getProviderFromModelCode('glm-4.6v-flash')).toBe('zhipu')
