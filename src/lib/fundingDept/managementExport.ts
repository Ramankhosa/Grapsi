...
export function exportTables(report:ManagementReport) {
  ...
  const opportunityGaps:unknown[][]=[['Member','School','Call ID','Call','Agency','Deadline','Days to deadline','Deadline status','Upcoming within 21 days','Missed without allocation or submission','Missed explanation','Any submission','Action state','Action signals','Gap flags','Recorded matches','Researchers reviewed','Researchers approached','Formal allocation','Match completeness','No-uptake reason']]
  ...
  const metadata:unknown[][]=[['Report date',report.asOf.toISOString()],['Mode',report.mode],['Period start',report.period.start.toISOString()],['Period end (exclusive)',report.period.end.toISOString()],['Upcoming definition','Confirmed relevant, due today through the next 21 India calendar days, with an open application or no formal allocation'],['Missed definition','Confirmed relevant, deadline passed, no formal allocation and no recorded submission'],['History complete',report.weekly.complete],['Matching complete',report.quality.matchingComplete]]
  metadata.push(['Active filters',JSON.stringify(report.filters)],['Workbench count unit','Duties; one school-call can have both origin review and researcher follow-up'])
  const performance:unknown[][]=[['Member','Opportunity workload','Acted-on opportunities','Untouched opportunities','Opportunity action coverage %','Outstanding applications','Allocations submitted','Allocation cohort','Submission conversion %','Faculty-contacted allocations','Allocation cohort','Contact coverage %','Timely actions','Dated completed actions','Timely action %','Sanctioned decisions','Decided submissions','Funding success %','Undecided submissions','Independent submissions','Period submissions','Previous equivalent period','Allocations performed','Contacts performed','Submissions recorded']]
