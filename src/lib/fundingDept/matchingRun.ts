...
export async function persistMatchingRun(input:{tenantId:string;callId:string;actorId?:string|null;orgUnitIds?:string[];filters:unknown;candidateCount:number;results:Array<{userId:string;score:number;matchTier:string;matchReason:string}>}) {
  ...
  const at=new Date();const version='researcher-search-gated-v1'
  await prisma.$transaction(async tx=>{
    await tx.$executeRaw(Prisma.sql`INSERT INTO dsr_matching_runs(id,tenant_id,call_id,actor_user_id,scope,version,completeness,result_count,candidate_count,results,completed_at)
      VALUES(${randomUUID()},${input.tenantId},${input.callId},${input.actorId||null},${JSON.stringify({schoolIds:schools,filters:input.filters,method:'bounded-top-k'})}::jsonb,${version},'PARTIAL',${observed.length},${input.candidateCount},${JSON.stringify(observed)}::jsonb,${at})`)
    for(const r of observed)await tx.$executeRaw(Prisma.sql`INSERT INTO funding_opportunity_matches(id,tenant_id,funding_call_id,user_id,org_unit_id,school_id,match_score,match_tier,match_reason,source,source_version,inferred,first_seen_at,last_seen_at,created_at,updated_at)
      VALUES(${randomUUID()},${input.tenantId},${input.callId},${r.userId},${r.orgUnitId},${r.schoolId},${r.score},${r.matchTier},${r.matchReason},'matching',${version},false,${at},${at},${at},${at})
      ON CONFLICT(tenant_id,funding_call_id,user_id,school_id) DO UPDATE SET last_seen_at=EXCLUDED.last_seen_at,updated_at=EXCLUDED.updated_at`)
  },{timeout:20000})
}
